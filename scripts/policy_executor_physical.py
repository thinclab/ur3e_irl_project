...
def ClaimNewOnion():
    ...
    ClaimNewOnion.userdata.sm_time = 0
    try:
        # Open the container
        with ClaimNewOnion:
            ...
            if multi_frame:
                ppv.StateMachine.add('GETINFO', ppv.Get_info_w_check(frame_threshold=3), 
                            ...
                            remapping={'x':'sm_x', 'y': 'sm_y', 'z': 'sm_z',
                                'color':'sm_color','counter':'sm_counter', 'time':'sm_time'	})
            else:
...
def main():
    ...
    parser.add_argument('--conveyor_speed', dest='conveyor_speed', default='0',
                help='Speed (in m/s) that the conveyor is moving at')
    parser.add_argument('--pick_time', dest='pick_time', default='5',
                help='Time in the future to pick an onion from when it was first detected (s)')
    ...
    ppv.pnp.conveyor_speed = float(args.conveyor_speed)
    ppv.pnp.pick_time = float(args.pick_time)

    print(ppv.pnp.conveyor_speed )
    print(ppv.pnp.pick_time)
    outcome = actList[4]()
    t2 = time()
